...
echo ""
echo "🎯 Step 4: Setting up samvad-local context..."
echo "   (kubectl config set-context samvad-local --namespace=samvad-dev --cluster=minikube --user=minikube)"
kubectl config set-context samvad-local --namespace=samvad-dev --cluster=minikube --user=minikube
kubectl config use-context samvad-local

echo ""
echo "🤫 Step 5: Pushing secrets (ssh, don't tell anyone)..."
echo "   (kubectl apply -f k8s/local/secret.yaml)"
kubectl apply -f k8s/local/secret.yaml

echo ""
echo "🤖 Step 6: Deploying the API (it's alive!)..."
echo "   (kubectl apply -f k8s/local/deployment.yaml)"
kubectl apply -f k8s/local/deployment.yaml

echo ""
echo "🌐 Step 7: Exposing the service..."
echo "   (kubectl apply -f k8s/local/service.yaml)"
kubectl apply -f k8s/local/service.yaml

echo ""
echo "🔌 Step 8: Opening the tunnel in the background — localhost:8080 ready!"
echo "   (kubectl port-forward service/samvad-api-service 8080:80 &)"
kubectl port-forward service/samvad-api-service 8080:80 > /dev/null 2>&1 &
